...
checkpoint_dir = '/data/lyli/Silencer/K562/datasets/6924/5fold/model/checkpoints_4layer_' + str(encode_epochs) + '-/No-attention'  # Store model files
if not os.path.exists(checkpoint_dir):  # if the path does not exist, generate it
    os.mkdir(checkpoint_dir)
...
if __name__ == "__main__":
    ...
    for train_index, test_index in folds.split(X_kmer, Y_kmer):
        ...
        if model_ind == "connection":
            ...
            saveBestModel = ModelCheckpoint(checkpoint_dir + "/model_noattention" + str(count_iter) + ".hdf5", monitor='val_acc',
            ...
            # serialize model to JSON
            model_json = model.to_json()
            with open("model_noattention" + str(count_iter) + ".json", "w") as json_file:
                json_file.write(model_json)
                # serialize weights to HDF5
            model.save_weights("model_noattention" + str(count_iter) + ".h5")
            print("Saved model to disk")
            ...
            dataframe.to_csv(checkpoint_dir + "/K562_our_method_ROC_noattention" + str(count_iter) + ".csv", index=False, sep=',')

            # 绘制PR所需的数据
            dataframe1 = pd.DataFrame({'precision': precision, 'recall': recall})
            dataframe1.to_csv(checkpoint_dir + "/K562_our_method_PR_noattention" + str(count_iter) + ".csv", index=False, sep=',')
    ...
    dataframe.to_csv(checkpoint_dir + "K562_our_method_ROC_noattention_average.csv", index=False, sep=',')