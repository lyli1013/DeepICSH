...
checkpoint_dir = '/data/lyli/Silencer/K562/datasets/6924/5fold/model/checkpoints_4layer_' + str(encode_epochs) + '-/omics_model'  # Store model files
if not os.path.exists(checkpoint_dir):  # if the path does not exist, generate it
    os.mkdir(checkpoint_dir)
...
if __name__ == "__main__":
    ...
    for train_index, test_index in folds.split(X_kmer, Y_kmer):
        ...
        if model_ind == "DNA":
            ...
            dataframe.to_csv(checkpoint_dir + "K562_only-seq_method" + str(count_iter) + ".csv", index=False, sep=',')
        ...
        if model_ind == "multi_omics":
            ...
            dataframe = pd.DataFrame({'真实标签': test_label, '预测概率': y_pred[:, 0]})
            dataframe.to_csv(checkpoint_dir + "K562_only-omics_method" + str(count_iter) + ".csv", index=False, sep=',')
    ...
    # dataframe.to_csv(checkpoint_dir + "K562_only-seq_method_average.csv", index=False, sep=',')
    dataframe.to_csv(checkpoint_dir + "K562_only-omics_method_average.csv", index=False, sep=',')
