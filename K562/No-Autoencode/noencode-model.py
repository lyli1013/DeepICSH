...
checkpoint_dir = '/data/lyli/Silencer/K562/datasets/6924/5fold/model/checkpoints_4layer_' + str(encode_epochs) + '-/No-encode'  # Store model files
if not os.path.exists(checkpoint_dir):  # if the path does not exist, generate it
    os.mkdir(checkpoint_dir)
...
if __name__ == "__main__":
    ...
    for train_index, test_index in folds.split(X_kmer, Y_kmer):
        ...
        if model_ind == "connection":
            ...
            # saveBestModel = ModelCheckpoint(checkpoint_dir + "/model-noencode" + str(count_iter) + ".hdf5", monitor='val_acc',
            ...
            model_first.load_weights(checkpoint_dir + "/model-noencode" + str(count_iter) + ".hdf5")
            # model_first.load_weights("/data/lyli/Silencer/HepG2/datasets/1679/12D/data_n=20/model_2merl.h5")
            ...
            dataframe.to_csv(checkpoint_dir + "/K562_our_method_model-noencode" + str(count_iter) + ".csv", index=False, sep=',')
    ...
    dataframe.to_csv(checkpoint_dir + "/K562_our_method_model-noencode_average.csv", index=False, sep=',')