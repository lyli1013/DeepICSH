...
def autoencode_train(count_iter, epochs, X_kmer_train):
    ...
    autoencoder.fit(X_kmer_train, X_kmer_train, epochs=epochs, batch_size=64, validation_split=0.1,
...
if __name__ == "__main__":
    ...
    for epoch in EPCOHS:
        checkpoint_dir = '/data/lyli/Silencer/K562/datasets/6924/5fold/second-data/model/autoencode'  # Store model files
        if not os.path.exists(checkpoint_dir):  # if the path does not exist, generate it
            os.mkdir(checkpoint_dir)
    
        checkpoint_dir = '/data/lyli/Silencer/K562/datasets/6924/5fold/second-data/model/autoencode/' + 'eopchis' + str(
            epoch)  # Store model files
        if not os.path.exists(checkpoint_dir):  # if the path does not exist, generate it
            os.mkdir(checkpoint_dir)
        # load kmer feature
        filename1 = "/data/lyli/Silencer/K562/datasets/6924/5fold/second-data/K562_second_kmer_data.txt"
