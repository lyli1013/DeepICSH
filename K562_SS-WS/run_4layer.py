...
checkpoint_dir = '/data/lyli/Silencer/K562/datasets/6924/5fold/second-data/model/checkpoints_4layer_' + str(encode_epochs)  # Store model files
if not os.path.exists(checkpoint_dir):  # if the path does not exist, generate it
    os.mkdir(checkpoint_dir)

checkpoint_dir = '/data/lyli/Silencer/K562/datasets/6924/5fold/second-data/model/checkpoints_4layer_' + str(encode_epochs) + '-/final_model'  # Store model files
if not os.path.exists(checkpoint_dir):  # if the path does not exist, generate it
    os.mkdir(checkpoint_dir)

encode_model = '/data/lyli/Silencer/K562/datasets/6924/5fold/second-data/model/autoencode'  # Store model files
if not os.path.exists(encode_model):  # if the path does not exist, generate it
    os.mkdir(encode_model)

encode_model = '/data/lyli/Silencer/K562/datasets/6924/5fold/second-data/model/autoencode/' + 'eopchis' + str(encode_epochs)  # Store model files
if not os.path.exists(encode_model):  # if the path does not exist, generate it
    os.mkdir(encode_model)


# load multi_omics data features
def load_omics_feature(filename1):
    filelist = [filename1]
    dataMat = []
...
if __name__ == "__main__":
    # load omics feature
    filename1 = "/data/lyli/Silencer/K562/datasets/6924/5fold/second-data/model/checkpoints_4layer_130-/independent_data/X_omics_train.txt"
    filename2 = "/data/lyli/Silencer/K562/datasets/6924/5fold/second-data/model/checkpoints_4layer_130-/independent_data/X_omics_test.txt"
    X_omics_train, Y_train = load_omics_feature(filename1)
    X_omics_test, Y_test = load_omics_feature(filename2)
    Y_train = np.array(Y_train)
    Y_test = np.array(Y_test)
    # X_omics, Y_omics = shuffle(X_omics, Y_omics, random_state=1)
    X_omics_train = X_omics_train.reshape(-1, 12, 1)
    X_omics_test = X_omics_test.reshape(-1, 12, 1)

    # load kmer feature
    filename3 = "/data/lyli/Silencer/K562/datasets/6924/5fold/second-data/model/checkpoints_4layer_130-/independent_data/X_kmer_train.txt"
    filename4 = "/data/lyli/Silencer/K562/datasets/6924/5fold/second-data/model/checkpoints_4layer_130-/independent_data/X_kmer_test.txt"
    X_kmer_train = load_kmer_feature(filename3)
    X_kmer_test = load_kmer_feature(filename4)
    X_kmer_train = np.array(X_kmer_train)
    X_kmer_test = np.array(X_kmer_test)
    # Y_kmer = [1] * 1000 + [0] * 1000
    # # print(np.shape(X_kmer))
    # Y_kmer = np.array(Y_kmer)
    # X_kmer, Y_kmer = shuffle(X_kmer, Y_kmer, random_state=1)
    X_kmer_train = X_kmer_train.reshape(-1, 1024, 1)
    X_kmer_test = X_kmer_test.reshape(-1, 1024, 1)
    # print(np.shape(X_kmer))
    ...
    #     X_omics_test = X_omics[test_index]
    #
    ...
    # 加载编码器模型
    encoder_model = load_model(encode_model + "/encoder_model_" + str(3) + 'fold' + '.hdf5')
    ...
    if model_ind == "DNA":
        ...
        saveBestModel = ModelCheckpoint(checkpoint_dir + "/model_DNA" + str(3) + ".hdf5", monitor='val_acc', verbose=1, save_best_only=True, mode='max')
        ...
        with open("model_DNA" + str(3) + ".json", "w") as json_file:
            json_file.write(model_json)
            # serialize weights to HDF5
        model.save_weights("model_DNA" + str(3) + ".h5")
        print("Saved model to disk")

        # load model
        model_first = Model_CNN.model_cnn(model_inds[0], EPCOHS, 128, 12)
        model_first.load_weights(checkpoint_dir + "/model_DNA" + str(3) + ".hdf5")
    ...
    if model_ind == "multi_omics":
        ...
        saveBestModel = ModelCheckpoint(checkpoint_dir + "/model_multi_omics" + str(3) + ".hdf5", monitor='val_acc', verbose=1, save_best_only=True, mode='max')
        callbacks_list = [saveBestModel]
        print('Fitting first model...')
        ...
        # serialize model to JSON
        model_json = model.to_json()
        with open("model_multi_omics" + str(3) + ".json", "w") as json_file:
            json_file.write(model_json)
            # serialize weights to HDF5
        model.save_weights("model_multi_omics" + str(3) + ".h5")
        print("Saved model to disk")

        # load model
        model_first = Model_CNN.model_cnn(model_inds[1], EPCOHS, 128, 12)
        model_first.load_weights(checkpoint_dir + "/model_multi_omics" + str(3) + ".hdf5")
        # model_first.load_weights("/data/lyli/Silencer/HepG2/datasets/1679/12D/data_n=20/model_2merl.h5")
    ...
    if model_ind == "connection":
        ...
        model_first.load_weights(checkpoint_dir + "/model" + str(3) + ".hdf5")
        # model_first.load_weights("/data/lyli/Silencer/HepG2/datasets/1679/12D/data_n=20/model_2merl.h5")
        ...
        dataframe.to_csv(checkpoint_dir + "K562-second_our_method_ROC" + str(3) + ".csv", index=False, sep=',')

        # 绘制PR所需数据
        dataframe1 = pd.DataFrame({'precision': precision, 'recall': recall})
        dataframe1.to_csv(checkpoint_dir + "K562-second_our_method_PR" + str(3) + ".csv", index=False, sep=',')
    ...
    # dataframe.to_csv(checkpoint_dir + "HepG2_our_only-omics_method_average.csv", index=False, sep=',')
    # dataframe.to_csv(checkpoint_dir + "HepG2_our_method_average.csv", index=False, sep=',')