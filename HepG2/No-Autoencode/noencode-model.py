...
checkpoint_dir = '/data/lyli/Silencer/HepG2/datasets/1679/12D/data_n=20/model/checkpoints_4layer_' + str(encode_epochs) + '-' + '/No-encode'  # Store model files
if not os.path.exists(checkpoint_dir):  # if the path does not exist, generate it
    os.mkdir(checkpoint_dir)
...
def evaluation(y1, y2):
    ...
    return ACC, Sens, Spec, MCC, AUC, PRC_AUC
...
if __name__ == "__main__":
    ...
    for train_index, test_index in folds.split(X_kmer, Y_kmer):
        ...
        kmer_train_X = X_kmer_train.reshape(-1, 1024)
        kmer_test_X = X_kmer_test.reshape(-1, 1024)
        ...
        if model_ind == "connection":
            tic = time.time()
            model = Model_CNN.model_cnn(model_inds[2], EPCOHS, 1024, 12)  # Get the first layer model
            saveBestModel = ModelCheckpoint(checkpoint_dir + "/model_noencode" + str(count_iter) + ".hdf5", monitor='val_acc', verbose=1, save_best_only=True, mode='max')
            callbacks_list = [saveBestModel]
            print('Fitting first model...')
            history = model.fit([kmer_train_X, X_omics_train], Y_train, batch_size=BATCH_SIZE, epochs=EPCOHS, callbacks=callbacks_list, validation_split=0.1, verbose=1)  # Start training
            # serialize model to JSON
            model_json = model.to_json()
            with open("model_noencode.json", "w") as json_file:
                json_file.write(model_json)
                # serialize weights to HDF5
            model.save_weights("model_noencode.h5")
            print("Saved model to disk")

            # load model
            model_first = Model_CNN.model_cnn(model_inds[2], EPCOHS, 1024, 12)
            # model_first.load_weights(checkpoint_dir + "/model_connection" + str(count_iter) + ".hdf5")
            model_first.load_weights(checkpoint_dir + "/model_noencode" + str(count_iter) + ".hdf5")
            # model_first.load_weights("/data/lyli/Silencer/HepG2/datasets/1679/12D/data_n=20/model_2merl.h5")
            y_pred = model_first.predict([kmer_test_X, X_omics_test], batch_size=BATCH_SIZE)
            ...
            ACC, Sens, Spec, MCC, ROC_AUC, PRC_ROC = evaluation(y_pred, test_label[0:669])
            print('ACC:', ACC)
            print('Sens:', Sens)
            print('Spec:', Spec)
            print('MCC:', MCC)
            ...
            dataframe.to_csv(checkpoint_dir + "/HepG2_our_method_noencode" + str(count_iter) + ".csv", index=False, sep=',')
    ...
    print('average_time:', sum(times) / 5)
    # 将这五个数组相加
    ...
    dataframe.to_csv(checkpoint_dir + "/HepG2_our_method_noencode_average.csv", index=False, sep=',')
