...
checkpoint_dir = '/data/lyli/Silencer/HepG2/datasets/1679/12D/data_n=20/model/checkpoints_4layer_' + str(
    encode_epochs)  # Store model files
if not os.path.exists(checkpoint_dir):  # if the path does not exist, generate it
    os.mkdir(checkpoint_dir)

checkpoint_dir = '/data/lyli/Silencer/HepG2/datasets/1679/12D/data_n=20/model/checkpoints_4layer_' + str(
    encode_epochs) + '-/final_model'  # Store model files
if not os.path.exists(checkpoint_dir):  # if the path does not exist, generate it
    os.mkdir(checkpoint_dir)
...
encode_model = '/data/lyli/Silencer/HepG2/datasets/1679/12D/data_n=20/model/autoencode/' + 'eopchis' + str(
    encode_epochs)  # Store model files
if not os.path.exists(encode_model):  # if the path does not exist, generate it
    os.mkdir(encode_model)
...
if __name__ == "__main__":
    ...
    for train_index, test_index in folds.split(X_kmer, Y_kmer):
        ...
        if model_ind == "connection":
            ...
            # saveBestModel = ModelCheckpoint(checkpoint_dir + "/model_connection" + str(count_iter) + ".hdf5", monitor='val_acc', verbose=1, save_best_only=True, mode='max')
            # callbacks_list = [saveBestModel]
            # print('Fitting first model...')
            ...
            # # serialize model to JSON
            # model_json = model.to_json()
            # with open("model_connection.json", "w") as json_file:
            #     json_file.write(model_json)
            #     # serialize weights to HDF5
            # model.save_weights("model_connection.h5")
            # print("Saved model to disk")
            ...
            dataframe.to_csv(checkpoint_dir + "HepG2_our_method" + str(count_iter) + ".csv", index=False, sep=',')

            # 绘制PR所需的数据
            dataframe1 = pd.DataFrame({'precision': precision, 'recall': recall})
            dataframe1.to_csv(checkpoint_dir + "HepG2_our_method_PR" + str(count_iter) + ".csv", index=False, sep=',')
