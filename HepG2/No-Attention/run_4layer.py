...
checkpoint_dir = '/data/lyli/Silencer/HepG2/datasets/1679/12D/data_n=20/model/checkpoints_4layer_' + str(encode_epochs)  # Store model files
if not os.path.exists(checkpoint_dir):  # if the path does not exist, generate it
    os.mkdir(checkpoint_dir)

checkpoint_dir = '/data/lyli/Silencer/HepG2/datasets/1679/12D/data_n=20/model/checkpoints_4layer_' + str(encode_epochs) + '-/No-attention'  # Store model files
if not os.path.exists(checkpoint_dir):  # if the path does not exist, generate it
    os.mkdir(checkpoint_dir)

encode_model = '/data/lyli/Silencer/HepG2/datasets/1679/12D/data_n=20/model/autoencode'  # Store model files
if not os.path.exists(encode_model):  # if the path does not exist, generate it
    os.mkdir(encode_model)

encode_model = '/data/lyli/Silencer/HepG2/datasets/1679/12D/data_n=20/model/autoencode/' + 'eopchis' + str(encode_epochs)  # Store model files
if not os.path.exists(encode_model):  # if the path does not exist, generate it
    os.mkdir(encode_model)
...
if __name__ == "__main__":
    # load omics feature
    filename1 = "/data/lyli/Silencer/HepG2/datasets/1679/12D/data_n=20/Silencer/0HepG2_silencer_feature12_1679_lable.txt"
    filename2 = "/data/lyli/Silencer/HepG2/datasets/1679/12D/data_n=20/NS/HepG2_negative_feature1679_12_lable.txt"
    ...
    # load kmer feature
    filename1 = "/data/lyli/Silencer/HepG2/datasets/1679/12D/data_n=20/HepG2_kmer_data.txt"
    X_kmer = load_kmer_feature(filename1)
    X_kmer = np.array(X_kmer)
    Y_kmer = [1] * 1679 + [0] * 1679
    ...
    for train_index, test_index in folds.split(X_kmer, Y_kmer):
        ...
        X_omics_test = X_omics[test_index][:669]

        X_kmer_train = X_kmer[train_index]  # X_train_first是数组
        X_kmer_test = X_kmer[test_index][:669]

        Y_train = Y_kmer[train_index]
        Y_test = Y_kmer[test_index][:669]
        ...
        if model_ind == "DNA":
            ...
            saveBestModel = ModelCheckpoint(checkpoint_dir + "/model_DNA" + str(count_iter) + ".hdf5",
            ...
            with open("model_DNA.json", "w") as json_file:
                json_file.write(model_json)
                # serialize weights to HDF5
            model.save_weights("model_DNA.h5")
        ...
        if model_ind == "multi_omics":
            ...
            saveBestModel = ModelCheckpoint(checkpoint_dir + "/model_multi_omics" + str(count_iter) + ".hdf5",
                                            monitor='val_acc', verbose=1, save_best_only=True, mode='max')
            callbacks_list = [saveBestModel]
            print('Fitting first model...')
            history = model.fit([kmer_train_X, X_omics_train], Y_train, batch_size=BATCH_SIZE, epochs=EPCOHS,
            ...
            # serialize model to JSON
            model_json = model.to_json()
            with open("model_multi_omics.json", "w") as json_file:
                json_file.write(model_json)
                # serialize weights to HDF5
            model.save_weights("model_multi_omics.h5")
            print("Saved model to disk")
        ...
        if model_ind == "connection":
            ...
            saveBestModel = ModelCheckpoint(checkpoint_dir + "/model_noattention" + str(count_iter) + ".hdf5", monitor='val_acc', verbose=1, save_best_only=True, mode='max')
            callbacks_list = [saveBestModel]
            print('Fitting first model...')
            history = model.fit([kmer_train_X, X_omics_train], Y_train, batch_size=BATCH_SIZE, epochs=EPCOHS, callbacks=callbacks_list, validation_split=0.1, verbose=1)  # Start training
            # serialize model to JSON
            model_json = model.to_json()
            with open("model_noattention.json", "w") as json_file:
                json_file.write(model_json)
                # serialize weights to HDF5
            model.save_weights("model_noattention.h5")
            print("Saved model to disk")

            # load model
            model_first = Model_CNN.model_cnn(model_inds[2], EPCOHS, 128, 12)
            # model_first.load_weights(checkpoint_dir + "/model_connection" + str(count_iter) + ".hdf5")
            model_first.load_weights(checkpoint_dir + "/model_noattention" + str(count_iter) + ".hdf5")
            # model_first.load_weights("/data/lyli/Silencer/HepG2/datasets/1679/12D/data_n=20/model_2merl.h5")
            y_pred = model_first.predict([kmer_test_X, X_omics_test], batch_size=BATCH_SIZE)
            ...
            ACC, Sens, Spec, MCC, ROC_AUC, PRC_ROC, precision, recall = evaluation(y_pred, test_label[0:672])
            print('ACC:', ACC)
            print('Sens:', Sens)
            print('Spec:', Spec)
            print('MCC:', MCC)
            ...
            # 绘制ROC所需的数据
            dataframe = pd.DataFrame({'真实标签': test_label[0:669], '预测概率': y_pred[:, 0]})
            dataframe.to_csv(checkpoint_dir + "/HepG2_our_method_ROC_noattention" + str(count_iter) + ".csv", index=False, sep=',')

            # 绘制PR所需的数据
            dataframe1 = pd.DataFrame({'precision': precision, 'recall': recall})
            dataframe1.to_csv(checkpoint_dir + "/HepG2_our_method_PR_noattention" + str(count_iter) + ".csv", index=False, sep=',')
    ...
    # 将这五个数组相加
    y_pred_sum_array = np.sum([y_pred_5fold[0], y_pred_5fold[1], y_pred_5fold[2], y_pred_5fold[3], y_pred_5fold[4]],
                              axis=0)
    print(y_pred_sum_array.shape)  # 输出相加后数组的形状
    # print(y_pred_sum_array)
    dataframe = pd.DataFrame({'真实标签': test_label[0:669], '预测概率': y_pred_sum_array[:, 0]})
    dataframe.to_csv(checkpoint_dir + "/HepG2_our_method_PR_noattention_average.csv", index=False, sep=',')
